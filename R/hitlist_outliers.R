...
hitlist_outliers <- function(data = NULL, control = NULL,
  ...
  message(paste("Removing", length(subset), "control channels"))
  if (length(subset) > 0) {
    data1 <- data[, -subset]
  }
  else {
  ...
  hits_list <- list()
  for(k in unique(cdata$treatment)){
    hits_list_ <- list()
    tr <- cdata %>% dplyr::filter(treatment == k)
    for(i in unique(cdata$temperature)){
      y <- tr %>% dplyr::filter(temperature == i)
      y$is.outlier <- rep(FALSE, nrow(y))
      hits_list_[[i]] <- y
    }
    hits_list[[k]] <- hits_list_
  }

  hits_list <- mapply(function(a,b){mapply(function(x, y) {x$is.outlier <- x$id %in% y; x}, a, b, SIMPLIFY = FALSE)},
               hits_list, hits, SIMPLIFY = FALSE)
  hits_list <- do.call(rbind, lapply(hits_list, function(x) do.call(rbind, x)))
  hits_list <- hits_list[order(hits_list$id),]
  rownames(hits_list) <- 1:nrow(hits_list)
  hits_list <- full_join(hits_list, proteininfo, by = "id")
  ...
  NN <- hits_list %>% dplyr::group_by(id, treatment) %>% dplyr::filter(n_out == 0)
  NN$category <- "NN"
  ...
  hits_list <- hits_list %>% dplyr::group_by(id, treatment) %>% dplyr::filter(n_out >= 1)
  ...
  hitlist <- hitlist %>% dplyr::group_by(id, Condition) %>%
    dplyr::mutate(score_outlier = sum(ord_outlier*is.outlier)/(nb_hits*n_out))
  hitlist <- hitlist %>% dplyr::group_by(id, Condition) %>%
