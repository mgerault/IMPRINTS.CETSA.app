...
#' Although, this function will only join on the protein position and will try to join close
#' position together; i.e. it will not take into account the amino-acid sequence.
#' Indeed, this function was written to join peptides dataset that would contain summed peptides
#' fold-changes before and after a specific position.
#'
...
imprints_join_peptides <- function(data){
  data <- lapply(data, function(x){
    x$`Master Protein Accessions` <- unlist(lapply(strsplit(x$`Master Protein Accessions`, " "), function(x) x[1]))
  ...
  ord <- lapply(strsplit(data$`Positions in Master Proteins`, "\\[|\\]"),
                function(x) {z = strsplit(x[2], "~|-");
                             z = as.numeric(z[[1]])
                             z = sum(z);
                             z
                             }
