...
* Rstudio version > 1.0
* Dependent packages:  
"IMPRINTS.CETSA", "ggplot2",  "stringr", "tidyr", "magick", "shiny", "shinydashboard", "shinycssloaders", "DT", "rio", "purrr", "dplyr", "plyr", "shinyjs", "cowplot", "plotly", "pubmedR", "rentrez", "XML", "gridExtra", "rjson", "bibliometrix", "officer", "colourpicker", "openxlsx", "visNetwork", "STRINGdb", "EBImage", "clusterProfiler", "biomaRt", "enrichplot", "multtest"
 
Some packages are not from CRAN and hence will not be installed when you will install mineCETSAapp package :  
"IMPRINTS.CETSA", "STRINGdb", "EBImage", "clusterProfiler", "biomaRt", "enrichplot", "multtest". For the IMPRINTS.CETSA package, you'll need the last version from github. For the others, you can install them from BioConductor. For this, type these commands in the R console :
...
The package contains also some data from "Modulation of Protein-Interaction States through the Cell Cycle" from Dai Lingyun and al. 2018, that you can play with.  Let's take a look at the data called `drug_data`. `drug_data` is a list that contains two datasets : "elutriation" and "chemarrest". 
...
From `peptides_norm_diff`, you can search for potential cleaved peptides. If it is cleaved, it means that first peptides (or the last) from the proteins are more abundant than the other from the same protein. 
...
Then, you can join your datasets (can be more than 2). Usually, the exact same peptide sequence is not found between the datasets. Since the fold change are summed, we can join 'early sequences' protein together, same for 'late sequences'.
