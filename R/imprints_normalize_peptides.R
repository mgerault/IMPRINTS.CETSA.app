...
#'
#' Function to normalize your peptides data.
#'
...
imprints_normalize_peptides <- function(peptides_data){
  ...
  # prepare data shape in order to use ms_2D_normalization from mineCETSA package
  ...
  peptides_data <- mineCETSA::ms_2D_normalization(peptides_data)
  norm_directory_toremove <- list.files()[!(list.files() %in% norm_directory_toremove)]
  norm_directory_toremove <- stringr::str_subset(norm_directory_toremove, "peptides_data_\\d{6}_\\d{4}$")
  if(length(norm_directory_toremove) == 1){
    unlink(norm_directory_toremove, recursive = TRUE)
  }

  # putting back the data in original shape
  peptides_data$id <- unlist(lapply(stringr::str_split(peptides_data$sumPSMs, "_sep_"), function(x) x[1]))
  peptides_data$sumPSMs <- unlist(lapply(stringr::str_split(peptides_data$sumPSMs, "_sep_"), function(x) x[2]))
