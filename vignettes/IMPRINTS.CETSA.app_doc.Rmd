...
From you IMPRINTS dataset on the peptide level, you can look for potential cleaved proteins; i.e. proteins with peptides with greater abundance and/or stabilization than onthers. For example, you can have peptides located between the N-terminal position and a cleaved site with greater stabilization than the ones that are after the cleaved site and before the C-terminal. We call it RESP effect.
Using the function `imprints_cleaved_peptides`, you can compute p-value for each proteins telling how likely a protein is cleaved based on the computed potential cleaved site. For that, let's compute fold-changes from all peptides. For more details, check the documentation of the function `imprints_cleaved_peptides`.
...
This function returns a data frame that contains the proteins with RESP effect that passes the cutoffs and their respective potential cleaved sites. It also saves automatically the data frame as an excel file and a volcano plot from each treatment tested.

####  $\underline{5.\space Check\space the\space cleaved\space peptides}$

You can now sum all the fold changes from all the peptides before the cleaved site found and after, and this for all poteins found significant RESP effect. For this, you can use again the `imprints_sequence_peptides` function but this time, input some sequences. Here, we will do it only for DrugA.
