  ...
  if(run_enrich){
    ...
    if(nrow(enrich) == 0){
      ...
      enrich <- enrich[,c("preferredNames", "fdr", "description")] %>%
        ...
        dplyr::reframe(Genes = stringr::str_split(preferredNames, ",")[[1]])
      colnames(enrich)[1] <- "GOterm"

      enrich <- enrich %>%
        dplyr::ungroup() %>% dplyr::group_by(Genes) %>%
        dplyr::group_modify(~ {
          if(nrow(.x) > 1){
            y <- .x[which.min(.x$fdr),]
            return(y)
      ...
      enrich <- enrich %>%
        dplyr::right_join(data_genes, by = "Genes") %>%
        dplyr::rename(name = id)
      enrich$Genes <- NULL
