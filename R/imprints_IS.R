...
imprints_IS <- function(data, data_diff = NULL, ctrl, valid_val = NULL,
  ...
  diff_IS <- data_diff[,-stringr::str_which(colnames(data_diff), paste0("_", ctrl, "$"))]
  ...
  for(k in cond){# get top 2 biggest diff_IS
    ...
    diff_IS[[paste0("Top2_", k)]] <- apply(diff_IS[,stringr::str_which(colnames(diff_IS), paste0("_", k, "$"))],
                                           1,
    ...
    diff_IS[[paste0("IS_", k)]] <-
      apply(diff_IS[,stringr::str_which(colnames(diff_IS), paste0("_", k, "$"))], 1,
            function(d){
              top = d[length(d)]
  ...
  cutoff <- diff_IS_plot %>% dplyr::group_by(treatment) %>%
    ...
    dplyr::reframe(pval = find_cutoff(Fisher, BH),
                     IS_pos = IS_cutoff + median(IS[which(Fisher < quantile(Fisher, 0.5))], na.rm = TRUE),
                     IS_neg = -IS_cutoff - median(IS[which(Fisher < quantile(Fisher, 0.5))], na.rm = TRUE))
  ...
  for_categorize <- diff_IS[,stringr::str_which(colnames(diff_IS), "^id|Gene|^\\d{1,}|^pval37")]
  ...
  diff_IS_plot$coeff <- apply(diff_IS_plot[,stringr::str_which(colnames(diff_IS_plot), "^\\d{1,}")], 1,
                              function(x){
                                x <- abs(x)
  ...
  diff_IS[,stringr::str_which(colnames(diff_IS), "category")] <- apply(as.data.frame(diff_IS[,stringr::str_which(colnames(diff_IS), "category")]),
  ...
  if(nrow(diff_IS_plot) > 1 & length(cond) > 1){
    ...
    for(i in 1:length(vennlist)){
      ...
      score_info <- diff_IS[,c(1, stringr::str_which(colnames(diff_IS), "^IS_|^Fisher_"))]
