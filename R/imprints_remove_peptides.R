...
imprints_remove_peptides <- function(data, proteins = NULL, sequence){
  ...
  if(is.null(proteins)){
    proteins <- unique(gsub("\\s.*", "", data$`Master Protein Accessions`))
  }
  else{
    if(!all(proteins %in% gsub("\\s.*", "", data$`Master Protein Accessions`))){
  ...
  for(p in 1:length(proteins)){
    protein_idx <- which(gsub("\\s.*", "", data$`Master Protein Accessions`) == proteins[p])
    ...
    sequence_toremove <- as.numeric(strsplit(sequence_toremove, "-")[[1]])

    sequence_toremove <- lapply(strsplit(protein_sequence, "-|~"),
