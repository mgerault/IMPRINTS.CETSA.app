...
#' It is totally based on the function imprints_barplotting from the IMPRINTS.CETSA package.
      ...
      for(k in names(data)){
        ...
        if (!is.null(corrtable)) {
          corrtable <- corrtable[order(corrtable$correlation,
                                       decreasing = T), ]
          if (printBothName & !pfdatabase) {
            corrtable <- IMPRINTS.CETSA::ms_composite_ID_Gene_Protein(corrtable,
                                                      pfdatabase)
          }
          else if (printGeneName & !pfdatabase) {
            corrtable <- IMPRINTS.CETSA::ms_composite_ID_Gene(corrtable)
          }
          else {
            corrtable <- IMPRINTS.CETSA::ms_composite_ID_Protein(corrtable,
                                                 pfdatabase)
          }
