...
## Introduction  
The mineCETSAapp package totally depends on the IMPRINTS.CETSA package, written by Dai Lingyun. 
...
The main goal of mineCETSAapp package is to provide a "user-friendly" interface of the IMPRINTS.CETSA package thanks to a shiny app. 
The app contains 6 tabs :

1. IMPRINTS.CETSA analysis 
   * Do the whole analysis of your IMPRINTS MS-CETSA datasets 
...
* Rstudio version > 1.0
* Dependent packages:  
"IMPRINTS.CETSA", "STRINGdb", "EBImage", "ggplot2",  "stringr", "tidyr", "magick", "shiny", "shinydashboard", "shinycssloaders", "DT", "rio", "purrr", "dplyr", "plyr", "shinyjs", "cowplot", "plotly", "pubmedR", "rentrez", "XML", "gridExtra", "rjson", "bibliometrix", "officer", "colourpicker", "openxlsx" 
 
Three packages are not from CRAN and hence will not be installed when you will install mineCETSAapp package :  
"IMPRINTS.CETSA", "STRINGdb", "EBImage". For the IMPRINTS.CETSA package, you'll need the last version from github. For the two others, you can install them from BioConductor. For this, type this command in the R console :
...
For this, you'll need the function imprints_complex_mapping_sh (which is also based on imprints_complex_mapping from IMPRINTS.CETSA).
...
But you can also use the SR_CetsaHit function which define hits if a protein has a big enough Stability Rate (SR) and good enough p-value (an FDR can be set).

In order to use this function, you'll need the post normalization data. The caldiff output is not necessary since it can be calculated automatically. You just need to set an FDR and a cutoff for the SR but you can let the default value. For more details, check the documentation of the function.
...
Since we didn't put the caldiff data, it will calculate it. Then it will save all the results (volcano plots, file output, venndiagram) in file named by default 'Hits_analysis'.
...
In this example, we match every protein name found under the condition S with the word "cell". Then we search for articles which contains those words in the title and/or the abstract, written in english and published between 2021 and 2022. If condition was set to "", then it would have taken all the protein names from your hitlist. You can also set 'language' and 'year_rg' to NULL to not filter on those criterias; same if you set 'feat' to "".
...
mineCETSAapp also contains functions to analyse IMPRINTS-CETSA on the peptide level. Their names always start with 'imprints' and ends with 'peptides'.
One of the main question for example is the search for potential cleaved peptides.
...
####  $\underline{4.\space Searching\space for\space cleaved\space peptides}$

From the 'peptides_norm_diff', you can search for potential cleaved peptides. If it is cleaved, it means that first peptides (or the last) from the proteins are more abundant than the other from the same protein. 
For this, you can use the 'imprints_cleaved_peptides' function. For more details, look into the documentation of the same function.
...
This function returns a data frame that contains the proteins that are potentially cleaved and their respective potential cleaved sites.

####  $\underline{5.\space Check\space the\space cleaved\space peptides}$

You can now sum all the fold changes from all the peptides before the cleaved site found and after, and this for all poteins found to be potentially cleaved. For this, you can use again the 'imprints_sequence_peptides' function but this time, input some sequences. Here, we will do it only for DrugA.
...
You can now check the barplots obtained and see if the proteins found are cleaved.
In 'imprints_sequence_peptides', you can put any sequence. Check the documentation from the function for more details.

####  $\underline{6.\space Multiple\space dataset\space analysis}$

If you do the exact same analysis for another dataset, or here with Drug B; you'll maybe want to compare your results between each drug.

For more clarity, let's remove the peptide cleaved site.
```{r, eval=FALSE}
...
Then, you can join your datasets (can be more than 2). Usually, the exact same peptide sequence is not found between the datasets. Since the fold change are summed, we can join 'early sequence' protein together, same for 'late sequence'.
...
You can use the 'imprints_barplotting_app' function to plot your final results.
