...
#'
#' @param data Output data from ms_2D_average 
...
ms_2D_heatmap <- function(data, hit_summary, NN_data = NULL,
                          treatment, max_na = 0,
                          response = c("S", "D", "both"), 
  ...
  #datah$temperature <- str_replace_all(datah$temperature, "^36C$", "EXP")
  ...
  stripr <- which(grepl('strip-l', H$layout$name)) #strip-r correspond to the facet label
